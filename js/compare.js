...
function radarLabels(metrics, cx, cy, radius) { return metrics.map((m, index) => { const angle = -Math.PI / 2 + (Math.PI * 2 * index / metrics.length); const x = cx + Math.cos(angle) * (radius + 38); const y = cy + Math.sin(angle) * (radius + 28); const anchor = Math.cos(angle) > 0.35 ? 'start' : Math.cos(angle) < -0.35 ? 'end' : 'middle'; const label = m.label.replace('Maintenance Resilience', 'Maintenance').replace('Population Capacity', 'Population').replace('Resource Security', 'Resources').replace('Exposure Control', 'Exposure'); return `<text x="${x.toFixed(1)}" y="${y.toFixed(1)}" text-anchor="${anchor}">${label}</text>`; }).join(''); }
function renderCategoryOverview(result) { const metrics = result.attributes; const leader = baseForWinner(result, result.overallWinner); const summary = metrics.map((m) => `${m.label}: ${result.baseA.name} ${formatScore(m.baseAValue)}, ${result.baseB.name} ${formatScore(m.baseBValue)}`).join('; '); const textSummary = metrics.map((m) => `<li>${m.label}: ${formatScore(m.baseAValue)} vs ${formatScore(m.baseBValue)}</li>`).join(''); const radarMarkup = metrics.length >= 3 ? `<figure class="radar-card"><svg class="comparison-radar" viewBox="0 0 420 340" role="img" aria-labelledby="radar-title radar-desc"><title id="radar-title">Category radar chart</title><desc id="radar-desc">${summary}</desc>${radarGrid(metrics,210,165,105)}<polygon points="${radarPoints(metrics,'baseA',210,165,105)}" class="radar-fill-a"/><polygon points="${radarPoints(metrics,'baseB',210,165,105)}" class="radar-fill-b"/>${radarLabels(metrics,210,165,105)}</svg><figcaption><span><i class="legend-a"></i>${result.baseA.name}</span><span><i class="legend-b"></i>${result.baseB.name}</span></figcaption></figure>` : `<div class="radar-card"><p class="winner-card-label">Category summary</p><ul>${textSummary}</ul></div>`; elements.categoryOverview.innerHTML = `<div class="category-count-card"><p class="winner-card-label">At a glance</p><div class="category-counts"><strong>${result.baseAWins}<span>${result.baseA.name} wins</span></strong><strong>${result.ties}<span>${result.ties === 1 ? 'Tie' : 'Ties'}</span></strong><strong>${result.baseBWins}<span>${result.baseB.name} wins</span></strong></div><p>${leader ? `${leader.name} leads the weighted result.` : 'The weighted overall score is even.'}</p></div>${radarMarkup}`; }
...
window.zombieBasesComparison = { SCORE_ROWS, buildComparisonResult, buildBaseComparison: buildComparisonResult, normaliseScore, formatScore, formatDifference };
initComparison();
})();
