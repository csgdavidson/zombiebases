...
function normaliseScore(value) { const score = Number(value); if (!Number.isFinite(score)) return null; return Math.max(0, Math.min(MAX_SCORE, score)); }
function isValidScoreValue(value) { return normaliseScore(value) !== null; }
function formatScore(value) { const score = normaliseScore(value); return score !== null ? score.toFixed(1) : '—'; }
function formatDifference(value) { const score = normaliseScore(value); return score === null || Math.abs(score) < WIN_EPSILON ? 'Even' : `+${Math.abs(score).toFixed(1)}`; }
function scoreToneClass(value) { const score = normaliseScore(value); if (score === null) return ''; if (score >= 8) return 'score-high'; if (score >= 5) return 'score-medium'; return 'score-low'; }
function scoreLabel(value) { const score = normaliseScore(value); if (score === null) return 'Unrated'; if (score >= 8.5) return 'Exceptional'; if (score >= 7) return 'Excellent'; if (score >= 5.5) return 'Good'; return 'Balanced'; }
...
function getScore(base, rowOrKey) { const row = typeof rowOrKey === 'string' ? SCORE_ROWS.find((item) => item.key === rowOrKey) : rowOrKey; if (!row) return null; if (row.source === 'overall') return normaliseScore(base?.scores?.overall); if (row.source === 'category') return normaliseScore(base?.scores?.categories?.[row.key]); return normaliseScore(base?.comparisonScores?.[row.key]?.score); }
...
function marginLabel(metric, result) { if (!isValidScoreValue(metric.decisiveDifference) || metric.winner === 'tie') return '<span>Even</span>'; const base = baseForWinner(result, metric.winner); return `<span>${displayMargin(metric.absoluteDifference)}</span><small>${shortName(base)} leads</small>`; }
function explanationFor(result) { if (result.overallWinner === 'tie') return 'The bases are level on weighted overall score, so the category profile determines which mission each location suits best.'; const winner = baseForWinner(result, result.overallWinner); const loser = result.overallWinner === 'baseA' ? result.baseB : result.baseA; const primary = result.overallWinner === 'baseA' ? result.largestBaseAAdvantage : result.largestBaseBAdvantage; const loserEdge = result.overallWinner === 'baseA' ? result.largestBaseBAdvantage : result.largestBaseAAdvantage; const categoryUpset = result.overallWinner === 'baseA' && result.baseBWins > result.baseAWins || result.overallWinner === 'baseB' && result.baseAWins > result.baseBWins; const edgeCopy = primary ? ` Its strongest supporting edge is ${primary.label.toLowerCase()} (${displayMargin(primary.absoluteDifference)}).` : ''; const counterCopy = loserEdge ? ` ${loser.name} is stronger in ${loserEdge.label.toLowerCase()}, so this remains a close comparison.` : ''; const categoryCopy = categoryUpset ? ` Although ${loser.name} leads more individual categories, ${winner.name}'s advantages carry slightly more weight in the overall model.` : ''; return `${winner.name} ${result.overallMargin <= 0.5 ? 'narrowly edges' : 'wins'} the weighted result by ${displayMargin(result.overallMargin)} points.${categoryCopy}${edgeCopy}${counterCopy}`; }
...
function displayMargin(value) { const score = normaliseScore(value); return score !== null ? `+${Math.abs(score).toFixed(1)}` : 'Even'; }
...
function summaryContext(result, winner) { if (!winner) return ''; const loser = result.overallWinner === 'baseA' ? result.baseB : result.baseA; const loserWinsMore = result.overallWinner === 'baseA' ? result.baseBWins > result.baseAWins : result.baseAWins > result.baseBWins; return loserWinsMore ? `${winner.name} wins the weighted result despite ${loser.name} leading more individual categories.` : `${winner.name} also leads the individual category count.`; }
function renderOverallSummary(result) { const winner = baseForWinner(result, result.overallWinner); const stats = winnerStats(result); const largest = result.largestAdvantage; const icon = '<svg viewBox="0 0 24 24" focusable="false"><path d="M6 4h12v3a6 6 0 0 1-5 5.92V16h3v2H8v-2h3v-3.08A6 6 0 0 1 6 7V4Zm2 2v1a4 4 0 0 0 8 0V6H8Zm-5 1h2a7.9 7.9 0 0 0 2 5.3C4.7 11.7 3 9.6 3 7Zm16 0h2c0 2.6-1.7 4.7-4 5.3A7.9 7.9 0 0 0 19 7Z"/></svg>'; elements.overallSummary.className = `overall-winner-summary ${winner ? 'has-winner' : 'is-tie'}`; elements.overallSummary.innerHTML = winner ? `<div class="summary-icon" aria-hidden="true">${icon}</div><div class="summary-main"><p class="eyebrow">Overall winner</p><h2>${winner.name}</h2><p class="base-meta">Wins by ${displayMargin(result.overallMargin)} weighted overall points</p></div><div class="summary-stats"><strong>${stats.wins}<span>Wins</span></strong><strong>${result.ties}<span>${result.ties === 1 ? 'Tie' : 'Ties'}</span></strong><strong>${stats.losses}<span>Losses</span></strong></div><div class="summary-copy"><p>${summaryContext(result, winner)}</p><p><strong>Largest winning advantage</strong><br>${largest ? `${largest.label} (${displayMargin(largest.absoluteDifference)})` : 'No clear category advantage'}</p></div>` : `<div class="summary-main"><p class="eyebrow">Overall draw</p><h2>Overall draw</h2><p class="base-meta">The bases are level on weighted overall score.</p></div><div class="summary-stats"><strong>${result.baseAWins}<span>${result.baseA.name} wins</span></strong><strong>${result.ties}<span>${result.ties === 1 ? 'Tie' : 'Ties'}</span></strong><strong>${result.baseBWins}<span>${result.baseB.name} wins</span></strong></div>`; }
const ICONS = {
  trophy: '<svg viewBox="0 0 24 24" focusable="false"><path d="M6 4h12v3a6 6 0 0 1-5 5.92V16h3v2H8v-2h3v-3.08A6 6 0 0 1 6 7V4Zm2 2v1a4 4 0 0 0 8 0V6H8Zm-5 1h2a7.9 7.9 0 0 0 2 5.3C4.7 11.7 3 9.6 3 7Zm16 0h2c0 2.6-1.7 4.7-4 5.3A7.9 7.9 0 0 0 19 7Z"/></svg>',
  star: '<svg viewBox="0 0 24 24" focusable="false"><path d="m12 3 2.7 5.5 6 .9-4.3 4.2 1 5.9-5.4-2.8-5.4 2.8 1-5.9-4.3-4.2 6-.9L12 3Z"/></svg>',
  equal: '<svg viewBox="0 0 24 24" focusable="false"><path d="M5 8h14v3H5V8Zm0 5h14v3H5v-3Z"/></svg>',
  trend: '<svg viewBox="0 0 24 24" focusable="false"><path d="M4 17.5 9.5 12l3 3L20 7.5V13h2V4h-9v2h5.5l-6 6-3-3L2.6 16 4 17.5Z"/></svg>',
  layers: '<svg viewBox="0 0 24 24" focusable="false"><path d="m12 3 9 5-9 5-9-5 9-5Zm0 12 7.2-4 1.8 1-9 5-9-5 1.8-1 7.2 4Zm0 4 7.2-4 1.8 1-9 5-9-5 1.8-1 7.2 4Z"/></svg>',
  target: '<svg viewBox="0 0 24 24" focusable="false"><path d="M13 2v2.06A8 8 0 0 1 19.94 11H22v2h-2.06A8 8 0 0 1 13 19.94V22h-2v-2.06A8 8 0 0 1 4.06 13H2v-2h2.06A8 8 0 0 1 11 4.06V2h2Zm-1 4a6 6 0 1 0 0 12 6 6 0 0 0 0-12Zm0 3a3 3 0 1 1 0 6 3 3 0 0 1 0-6Z"/></svg>',
  info: '<svg viewBox="0 0 24 24" focusable="false"><path d="M11 10h2v7h-2v-7Zm0-3h2v2h-2V7Zm1-5a10 10 0 1 0 0 20 10 10 0 0 0 0-20Zm0 18a8 8 0 1 1 0-16 8 8 0 0 1 0 16Z"/></svg>'
};
function pluralise(count, singular, plural = `${singular}s`) { return `${count} ${count === 1 ? singular : plural}`; }
function categoryWinnerSide(result) { if (result.baseAWins > result.baseBWins) return 'baseA'; if (result.baseBWins > result.baseAWins) return 'baseB'; return 'tie'; }
function categorySummaryItems(result, winnerSide = categoryWinnerSide(result)) { if (winnerSide === 'baseA') return [{ tone: 'baseA', text: pluralise(result.baseAWins, 'win') }, { tone: 'neutral', text: pluralise(result.ties, 'tie') }, { tone: 'baseB', text: pluralise(result.baseBWins, 'loss', 'losses') }]; if (winnerSide === 'baseB') return [{ tone: 'baseB', text: pluralise(result.baseBWins, 'win') }, { tone: 'neutral', text: pluralise(result.ties, 'tie') }, { tone: 'baseA', text: pluralise(result.baseAWins, 'loss', 'losses') }]; return [{ tone: 'baseA', text: `${result.baseAWins} ${shortName(result.baseA)} ${result.baseAWins === 1 ? 'win' : 'wins'}` }, { tone: 'baseB', text: `${result.baseBWins} ${shortName(result.baseB)} ${result.baseBWins === 1 ? 'win' : 'wins'}` }, { tone: 'neutral', text: pluralise(result.ties, 'tie') }]; }
function categorySummaryText(result, winnerSide = categoryWinnerSide(result)) { return categorySummaryItems(result, winnerSide).map((item) => item.text).join(' · '); }
function categorySummaryMarkup(result, winnerSide = categoryWinnerSide(result)) { return categorySummaryItems(result, winnerSide).map((item) => `<span><i class="result-dot dot-${item.tone}"></i>${item.text}</span>`).join(''); }
function weightedResultCopy(result, categorySide = categoryWinnerSide(result)) { const weighted = baseForWinner(result, result.overallWinner); if (!weighted) return 'The weighted overall result is tied.'; if (categorySide === result.overallWinner) return `${weighted.name} also leads the weighted overall result.`; return `${weighted.name} still leads the weighted overall result.`; }
function insightMargin(value) { const score = Number(value); return Number.isFinite(score) ? `+${Math.abs(score).toFixed(1)}` : 'Even'; }
function categoryDifferences(metrics) { return metrics.filter((m) => isValidScoreValue(m.baseAValue) && isValidScoreValue(m.baseBValue)).map((m) => ({ ...m, difference: m.baseAValue - m.baseBValue })); }
function categoryInsights(result) {
  const diffs = categoryDifferences(result.attributes);
  if (!diffs.length) return [];
  const byAbs = (best, item) => !best || Math.abs(item.difference) < Math.abs(best.difference) ? item : best;
  const baseA = diffs.filter((item) => item.difference > WIN_EPSILON).reduce((best, item) => !best || item.difference > best.difference ? item : best, null);
  const baseB = diffs.filter((item) => item.difference < -WIN_EPSILON).reduce((best, item) => !best || item.difference < best.difference ? item : best, null);
  const closest = diffs.reduce(byAbs, null);
  const insights = [];
  if (baseA) insights.push({ tone: 'baseA', icon: 'trend', kicker: 'Biggest advantage', title: result.baseA.name, detail: `${baseA.label} (${insightMargin(baseA.difference)})` });
  if (baseB) insights.push({ tone: 'baseB', icon: 'layers', kicker: 'Biggest advantage', title: result.baseB.name, detail: `${baseB.label} (${insightMargin(baseB.difference)})` });
  if (closest) insights.push({ tone: 'neutral', icon: 'target', kicker: 'Closest match', title: closest.label, detail: Math.abs(closest.difference) < WIN_EPSILON ? 'Scores are tied' : 'Very similar performance' });
  return insights.length ? insights : [{ tone: 'neutral', icon: 'equal', kicker: 'Category profile', title: 'All categories are tied', detail: 'Neither base has a category advantage.' }];
}
function radarPoints(metrics, side, cx, cy, radius) { return metrics.map((m, index) => { const angle = -Math.PI / 2 + (Math.PI * 2 * index / metrics.length); const score = normaliseScore(side === 'baseA' ? m.baseAValue : m.baseBValue) || 0; const r = radius * (score / MAX_SCORE); return `${(cx + Math.cos(angle) * r).toFixed(1)},${(cy + Math.sin(angle) * r).toFixed(1)}`; }).join(' '); }
function radarPointCircles(metrics, side, cx, cy, radius) { return metrics.map((m, index) => { const angle = -Math.PI / 2 + (Math.PI * 2 * index / metrics.length); const score = normaliseScore(side === 'baseA' ? m.baseAValue : m.baseBValue) || 0; const r = radius * (score / MAX_SCORE); return `<circle cx="${(cx + Math.cos(angle) * r).toFixed(1)}" cy="${(cy + Math.sin(angle) * r).toFixed(1)}" r="3.7" class="radar-point-${side === 'baseA' ? 'a' : 'b'}"/>`; }).join(''); }
...
function radarAxes(metrics, cx, cy, radius) { return metrics.map((m, index) => { const angle = -Math.PI / 2 + (Math.PI * 2 * index / metrics.length); return `<line x1="${cx}" y1="${cy}" x2="${(cx + Math.cos(angle) * radius).toFixed(1)}" y2="${(cy + Math.sin(angle) * radius).toFixed(1)}" class="radar-axis"/>`; }).join(''); }
function radarLabels(metrics, cx, cy, radius) { return metrics.map((m, index) => { const angle = -Math.PI / 2 + (Math.PI * 2 * index / metrics.length); const x = cx + Math.cos(angle) * (radius + 52); const y = cy + Math.sin(angle) * (radius + 38); const anchor = Math.cos(angle) > 0.35 ? 'start' : Math.cos(angle) < -0.35 ? 'end' : 'middle'; const label = m.label.replace('Maintenance Resilience', 'Maintenance').replace('Population Capacity', 'Population').replace('Resource Security', 'Resources').replace('Exposure Control', 'Exposure'); return `<text x="${x.toFixed(1)}" y="${y.toFixed(1)}" text-anchor="${anchor}">${label}</text>`; }).join(''); }
function renderCategoryOverview(result) {
  const metrics = result.attributes;
  const categorySide = categoryWinnerSide(result);
  const categoryWinner = baseForWinner(result, categorySide);
  const headline = categoryWinner ? `${categoryWinner.name} wins the category comparison` : 'The category comparison is tied';
  const summary = metrics.map((m) => `${m.label}: ${result.baseA.name} ${formatScore(m.baseAValue)}, ${result.baseB.name} ${formatScore(m.baseBValue)}`).join('; ');
  const textRows = metrics.map((m) => `<tr><td>${m.label}</td><td>${formatScore(m.baseAValue)}</td><td>${formatScore(m.baseBValue)}</td></tr>`).join('');
  const summaryCards = [
    { key: 'baseA', value: result.baseAWins, label: `${result.baseA.name} wins`, description: `Outperforms in ${pluralise(result.baseAWins, 'category', 'categories')}`, tone: 'baseA', icon: 'trophy' },
    { key: 'ties', value: result.ties, label: 'Ties', description: result.ties === 1 ? 'One category is evenly matched' : 'Categories are evenly matched', tone: 'neutral', icon: 'equal' },
    { key: 'baseB', value: result.baseBWins, label: `${result.baseB.name} wins`, description: `Outperforms in ${pluralise(result.baseBWins, 'category', 'categories')}`, tone: 'baseB', icon: 'star' }
  ];
  const insights = categoryInsights(result);
  const radarMarkup = metrics.length >= 3 ? `<div class="category-chart-wrap"><svg class="comparison-radar" viewBox="0 0 560 520" role="img" aria-labelledby="radar-title radar-desc"><title id="radar-title">Category radar chart</title><desc id="radar-desc">${summary}</desc>${radarGrid(metrics,280,260,165)}${radarAxes(metrics,280,260,165)}<polygon points="${radarPoints(metrics,'baseA',280,260,165)}" class="radar-fill-a"/><polygon points="${radarPoints(metrics,'baseB',280,260,165)}" class="radar-fill-b"/>${radarPointCircles(metrics,'baseA',280,260,165)}${radarPointCircles(metrics,'baseB',280,260,165)}${radarLabels(metrics,280,260,165)}</svg></div>` : `<div class="category-chart-wrap category-chart-fallback"><ul>${metrics.map((m) => `<li>${m.label}: ${formatScore(m.baseAValue)} vs ${formatScore(m.baseBValue)}</li>`).join('')}</ul></div>`;
  elements.categoryOverview.className = 'category-overview-redesign';
  elements.categoryOverview.innerHTML = `<div class="category-winner-header"><div class="category-result-icon" aria-hidden="true">${categoryWinner ? ICONS.trophy : ICONS.equal}</div><div><p class="eyebrow">Category overview</p><h3>${headline}</h3><p class="category-compact-summary">${categorySummaryMarkup(result, categorySide)}</p></div></div><div class="weighted-result-callout"><span aria-hidden="true">${ICONS.star}</span><p>${weightedResultCopy(result, categorySide)}</p></div><div class="category-stat-grid">${summaryCards.map((card) => `<article class="category-stat-card tone-${card.tone}"><div><strong>${card.value}</strong><h4>${card.label}</h4><p>${card.description}</p></div><span class="category-stat-icon" aria-hidden="true">${ICONS[card.icon]}</span></article>`).join('')}</div><section class="category-chart-panel" aria-labelledby="category-chart-heading"><div class="category-panel-copy"><p class="winner-card-label" id="category-chart-heading">Category comparison</p><p>Each axis represents a survival factor. Farther from the centre means stronger performance.</p><div class="category-insights">${insights.map((item) => `<article class="category-insight tone-${item.tone}"><span aria-hidden="true">${ICONS[item.icon]}</span><div><p>${item.kicker}</p><strong>${item.title}</strong><small>${item.detail}</small></div></article>`).join('')}</div></div><div class="category-panel-chart">${radarMarkup}<div class="comparison-legend" aria-label="Chart legend"><span><i class="legend-a"></i>${result.baseA.name}</span><span><i class="legend-b"></i>${result.baseB.name}</span></div></div><table class="sr-only"><caption>Category score comparison</caption><thead><tr><th>Category</th><th>${result.baseA.name}</th><th>${result.baseB.name}</th></tr></thead><tbody>${textRows}</tbody></table></section><footer class="category-scoring-footer"><span aria-hidden="true">${ICONS.info}</span><p>The weighted overall score also considers the importance of each category.</p><a href="/field-manual#how-zombie-bases-scores-every-location">How scoring works →</a></footer>`;
}
function renderScoreRows(result) { elements.scoreBody.innerHTML = ''; elements.scoreBaseA.textContent = result.baseA.name; elements.scoreBaseB.textContent = result.baseB.name; result.metrics.forEach((metric) => { const aWin = metric.winner === 'baseA'; const bWin = metric.winner === 'baseB'; const item = document.createElement('article'); item.className = `score-comparison-row${metric.key === 'overall' ? ' is-overall' : ''}`; const aWidth = Math.max(0, Math.min(100, ((metric.baseAValue || 0) / MAX_SCORE) * 100)); const bWidth = Math.max(0, Math.min(100, ((metric.baseBValue || 0) / MAX_SCORE) * 100)); item.innerHTML = `<div class="score-metric"><strong>${metric.label}</strong><small>${metric.description || ''}</small></div><div class="score-bar-cell"><span class="score-mobile-base-name">${result.baseA.name}</span><span class="score-num ${aWin ? 'score-cell-winner' : ''}">${formatScore(metric.baseAValue)}</span><div class="score-track" role="img" aria-label="${result.baseA.name} ${metric.label}: ${formatScore(metric.baseAValue)} out of 10, ${aWin ? 'leads' : bWin ? 'trails' : 'ties'}"><span class="score-fill score-fill-a" style="width:${aWidth}%"></span></div></div><div class="score-margin ${metric.winner === 'tie' ? 'score-difference-even' : 'score-difference-cell'}">${marginLabel(metric, result)}</div><div class="score-bar-cell score-bar-cell-b"><span class="score-mobile-base-name">${result.baseB.name}</span><span class="score-num ${bWin ? 'score-cell-winner' : ''}">${formatScore(metric.baseBValue)}</span><div class="score-track" role="img" aria-label="${result.baseB.name} ${metric.label}: ${formatScore(metric.baseBValue)} out of 10, ${bWin ? 'leads' : aWin ? 'trails' : 'ties'}"><span class="score-fill score-fill-b" style="width:${bWidth}%"></span></div></div>`; elements.scoreBody.appendChild(item); }); }
...
function renderRecommendation(result) { const winner = baseForWinner(result, result.overallWinner); const reasons = recommendationReasons(result); const alt = alternativeNote(result); elements.recommendation.innerHTML = winner ? `<div class="recommendation-icon" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><path d="M12 3l7 3v5c0 4.4-2.8 8.4-7 10-4.2-1.6-7-5.6-7-10V6l7-3Zm0 2.2L7 7.35V11c0 3.15 1.9 6.15 5 7.75 3.1-1.6 5-4.6 5-7.75V7.35l-5-2.15Z"/></svg></div><div><p class="eyebrow">Recommendation</p><h3>${winner.name} ${result.overallMargin <= 0.5 ? 'narrowly wins.' : 'is the stronger overall choice.'}</h3><p>${explanationFor(result)}</p></div><ul>${reasons.length ? reasons.map((reason)=>`<li>${reason}</li>`).join('') : '<li>The weighted overall model gives this base the edge.</li>'}</ul>${alt ? `<p class="alternative-note">${alt}</p>` : ''}` : `<div><p class="eyebrow">Recommendation</p><h3>This matchup is an overall draw.</h3><p>Use the tied and category-winning factors above to choose the better mission fit.</p></div>`; }
function renderStrengths(result) { elements.strengths.innerHTML = [ ['baseA', result.baseA], ['baseB', result.baseB] ].map(([side, base]) => { const wins = result.attributes.filter((m) => m.winner === side && (m.absoluteDifference || 0) >= 1).sort((a,b)=>(b.absoluteDifference||0)-(a.absoluteDifference||0)).slice(0,3); const losses = result.attributes.filter((m) => m.winner !== side && m.winner !== 'tie' && (m.absoluteDifference || 0) >= 1).sort((a,b)=>(b.absoluteDifference||0)-(a.absoluteDifference||0)).slice(0,2); return `<article class="tradeoff-card ${side === result.overallWinner ? 'is-overall-winner' : ''}"><div class="tradeoff-card-head"><span class="tradeoff-icon" aria-hidden="true">${side === 'baseA' ? '♜' : '♖'}</span><h3>${base.name}</h3></div><div class="tradeoff-columns"><div><p class="winner-card-label">Strengths</p><ul>${wins.length ? wins.map((m)=>`<li>${leadershipLabel(m)} in ${m.label} by ${formatDifference(m.decisiveDifference)}.</li>`).join('') : '<li>No major category strengths.</li>'}</ul></div><div><p class="winner-card-label">Trade-offs</p><ul>${losses.length ? losses.map((m)=>`<li>Trails in ${m.label} by ${formatDifference(m.decisiveDifference)}.</li>`).join('') : '<li>No major scoring weaknesses.</li>'}</ul></div></div></article>`; }).join(''); }
...
window.zombieBasesComparison = { SCORE_ROWS, buildComparisonResult, buildBaseComparison: buildComparisonResult, normaliseScore, formatScore, formatDifference, categoryWinnerSide, categorySummaryText, categorySummaryItems, weightedResultCopy, categoryInsights, pluralise, insightMargin };
initComparison();
})();
